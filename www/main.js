(() => {
  ...
  var require_paper_full = __commonJS({
    "node_modules/paper/dist/paper-full.js"(exports, module) {
      var paper2 = function(self2, undefined) {
        ...
        var PaperScope = Base.extend({
          ...
          execute: function(code, options) {
            var exports2 = paper3.PaperScript.execute(code, this, options);
            View.updateFocus();
            return exports2;
          },
        ...
        var Numerical = new function() {
          ...
          return {
            ...
            solveCubic: function(a, b, c, d3, roots, min, max) {
              var f = getNormalizationFactor(abs(a), abs(b), abs(c), abs(d3)), x, b1, c2, qd, q;
              if (f) {
                a *= f;
                b *= f;
                c *= f;
        ...
        var Point = Base.extend({
          ...
          getDistance: function() {
            var args = arguments, point = Point.read(args), x = point.x - this.x, y = point.y - this.y, d3 = x * x + y * y, squared = Base.read(args);
            return squared ? d3 : Math.sqrt(d3);
          },
        ...
        var Matrix = Base.extend({
          ...
          set: "#initialize",
          _set: function(a, b, c, d3, tx, ty, _dontNotify) {
            this._a = a;
            this._b = b;
            this._c = c;
            this._d = d3;
          ...
          rotate: function(angle) {
            angle *= Math.PI / 180;
            var center = Point.read(arguments, 1), x = center.x, y = center.y, cos = Math.cos(angle), sin = Math.sin(angle), tx = x - x * cos + y * sin, ty = y - x * sin - y * cos, a = this._a, b = this._b, c = this._c, d3 = this._d;
            this._a = cos * a + sin * c;
            this._b = cos * b + sin * d3;
            this._c = -sin * a + cos * c;
            this._d = -sin * b + cos * d3;
            this._tx += tx * a + ty * c;
            this._ty += tx * b + ty * d3;
            this._changed();
            return this;
          ...
          append: function(mx, _dontNotify) {
            if (mx) {
              var a1 = this._a, b1 = this._b, c1 = this._c, d12 = this._d, a2 = mx._a, b2 = mx._c, c2 = mx._b, d22 = mx._d, tx2 = mx._tx, ty2 = mx._ty;
              this._a = a2 * a1 + c2 * c1;
              this._c = b2 * a1 + d22 * c1;
              this._b = a2 * b1 + c2 * d12;
              this._d = b2 * b1 + d22 * d12;
              this._tx += tx2 * a1 + ty2 * c1;
              this._ty += tx2 * b1 + ty2 * d12;
              if (!_dontNotify)
                this._changed();
            }
            return this;
          ...
          prepend: function(mx, _dontNotify) {
            if (mx) {
              var a1 = this._a, b1 = this._b, c1 = this._c, d12 = this._d, tx1 = this._tx, ty1 = this._ty, a2 = mx._a, b2 = mx._c, c2 = mx._b, d22 = mx._d, tx2 = mx._tx, ty2 = mx._ty;
              this._a = a2 * a1 + b2 * b1;
              this._c = a2 * c1 + b2 * d12;
              this._b = c2 * a1 + d22 * b1;
              this._d = c2 * c1 + d22 * d12;
              this._tx = a2 * tx1 + b2 * ty1 + tx2;
              this._ty = c2 * tx1 + d22 * ty1 + ty2;
              if (!_dontNotify)
                this._changed();
            }
            return this;
          ...
          invert: function() {
            var a = this._a, b = this._b, c = this._c, d3 = this._d, tx = this._tx, ty = this._ty, det = a * d3 - b * c, res = null;
            if (det && !isNaN(det) && isFinite(tx) && isFinite(ty)) {
              this._a = d3 / det;
              this._b = -b / det;
              this._c = -c / det;
              this._d = a / det;
              this._tx = (c * ty - d3 * tx) / det;
          ...
          decompose: function() {
            var a = this._a, b = this._b, c = this._c, d3 = this._d, det = a * d3 - b * c, sqrt = Math.sqrt, atan2 = Math.atan2, degrees = 180 / Math.PI, rotate, scale, skew;
            if (a !== 0 || b !== 0) {
              var r = sqrt(a * a + b * b);
              rotate = Math.acos(a / r) * (b > 0 ? 1 : -1);
              scale = [r, det / r];
              skew = [atan2(a * c + b * d3, r * r), 0];
            } else if (c !== 0 || d3 !== 0) {
              var s = sqrt(c * c + d3 * d3);
              rotate = Math.asin(c / s) * (d3 > 0 ? 1 : -1);
              scale = [det / s, s];
              skew = [0, atan2(a * c + b * d3, s * s)];
            } else {
              rotate = 0;
              skew = scale = [0, 0];
            }
        ...
        var Item = Base.extend(
          ...
          }(),
          {
            ...
            _initialize: function(props, point) {
              var hasProps = props && Base.isPlainObject(props), internal = hasProps && props.internal === true, matrix = this._matrix = new Matrix(), project = hasProps && props.project || paper3.project, settings = paper3.settings;
        ...
        var Shape = Item.extend(
          {
            ...
            toPath: function(insert) {
              ...
              if (paper3.settings.applyMatrix)
        ...
        var HitResult = Base.extend({
          ...
          statics: {
            getOptions: function(args) {
              ...
              return new Base({
                type: null,
                tolerance: paper3.settings.hitTolerance,
                fill: !options,
                stroke: !options,
                segments: !options,
                handles: false,
        ...
        var Segment = Base.extend({
          ...
          smooth: function(options, _first, _last) {
            var opts = options || {}, type = opts.type, factor = opts.factor, prev = this.getPrevious(), next = this.getNext(), p0 = (prev || this)._point, p1 = this._point, p2 = (next || this)._point, d12 = p0.getDistance(p1), d22 = p1.getDistance(p2);
            if (!type || type === "catmull-rom") {
              var a = factor === undefined ? 0.5 : factor, d1_a = Math.pow(d12, a), d1_2a = d1_a * d1_a, d2_a = Math.pow(d22, a), d2_2a = d2_a * d2_a;
              if (!_first && prev) {
              ...
              if (prev && next) {
                var vector = p0.subtract(p2), t = factor === undefined ? 0.4 : factor, k = t * d12 / (d12 + d22);
                if (!_first)
          ...
          new function() {
            ...
            function evaluate(v, t, type, normalized) {
              ...
              if (type === 0) {
                ...
                if (type === 3) {
                  var x2 = 6 * ax * t + 2 * bx, y2 = 6 * ay * t + 2 * by, d3 = Math.pow(x * x + y * y, 3 / 2);
                  x = d3 !== 0 ? (x * y2 - y * x2) / d3 : 0;
                  y = 0;
                }
              }
            ...
            return { statics: {
              classify: function(v) {
                var x0 = v[0], y0 = v[1], x1 = v[2], y1 = v[3], x2 = v[4], y2 = v[5], x3 = v[6], y3 = v[7], a1 = x0 * (y3 - y2) + y0 * (x2 - x3) + x3 * y2 - y3 * x2, a2 = x1 * (y0 - y3) + y1 * (x3 - x0) + x0 * y3 - y0 * x3, a3 = x2 * (y1 - y0) + y2 * (x0 - x1) + x1 * y0 - y1 * x0, d3 = 3 * a3, d22 = d3 - a2, d12 = d22 - a2 + a1, l = Math.sqrt(d12 * d12 + d22 * d22 + d3 * d3), s = l !== 0 ? 1 / l : 0, isZero = Numerical.isZero, serpentine = "serpentine";
                d12 *= s;
                d22 *= s;
                d3 *= s;
                ...
                if (isZero(d12)) {
                  return isZero(d22) ? type(isZero(d3) ? "line" : "quadratic") : type(serpentine, d3 / (3 * d22));
                }
                var d4 = 3 * d22 * d22 - 4 * d12 * d3;
                if (isZero(d4)) {
                  return type("cusp", d22 / (2 * d12));
                }
                var f1 = d4 > 0 ? Math.sqrt(d4 / 3) : Math.sqrt(-d4), f2 = 2 * d12;
                return type(
                  d4 > 0 ? serpentine : "loop",
                  (d22 + f1) / f2,
                  (d22 - f1) / f2
                );
              },
          ...
          new function() {
            ...
            function addCurveIntersections(v1, v2, c1, c2, locations, include, flip, recursion, calls, tMin, tMax, uMin, uMax) {
              if (++calls >= 4096 || ++recursion >= 40)
                return calls;
              var fatLineEpsilon = 1e-9, q0x = v2[0], q0y = v2[1], q3x = v2[6], q3y = v2[7], getSignedDistance = Line.getSignedDistance, d12 = getSignedDistance(q0x, q0y, q3x, q3y, v2[2], v2[3]), d22 = getSignedDistance(q0x, q0y, q3x, q3y, v2[4], v2[5]), factor = d12 * d22 > 0 ? 3 / 4 : 4 / 9, dMin = factor * Math.min(0, d12, d22), dMax = factor * Math.max(0, d12, d22), dp0 = getSignedDistance(q0x, q0y, q3x, q3y, v1[0], v1[1]), dp1 = getSignedDistance(q0x, q0y, q3x, q3y, v1[2], v1[3]), dp2 = getSignedDistance(q0x, q0y, q3x, q3y, v1[4], v1[5]), dp3 = getSignedDistance(q0x, q0y, q3x, q3y, v1[6], v1[7]), hull = getConvexHull(dp0, dp1, dp2, dp3), top = hull[0], bottom = hull[1], tMinClip, tMaxClip;
              if (d12 === 0 && d22 === 0 && dp0 === 0 && dp1 === 0 && dp2 === 0 && dp3 === 0 || (tMinClip = clipConvexHull(top, bottom, dMin, dMax)) == null || (tMaxClip = clipConvexHull(
                top.reverse(),
                bottom.reverse(),
                dMin,
                dMax
            ...
            function getTimesWithTangent(v, tangent) {
              ...
              if (Math.abs(den) < Numerical.CURVETIME_EPSILON) {
                ...
                if (delta >= 0 && den != 0) {
                  var d3 = Math.sqrt(delta), t0 = -(k + d3) / den, t1 = (-k + d3) / den;
          ...
          new function() {
            function insert(locations, loc, merge) {
              ...
              while (l <= r) {
                ...
                var path12 = loc.getPath(), path22 = loc2.getPath(), diff = path12 !== path22 ? path12._id - path22._id : loc.getIndex() + loc.getTime() - (loc2.getIndex() + loc2.getTime());
                if (diff < 0) {
                  r = m - 1;
                } else {
                  l = m + 1;
          ...
          compare: function(path) {
            ...
            if (path) {
              ...
              for (var i1 = length1 - 1; i1 >= 0 && ok; i1--) {
                var path12 = paths1[i1];
                ok = false;
                var pathBoundsOverlaps = boundsOverlaps[i1];
                if (pathBoundsOverlaps) {
                  for (var i2 = pathBoundsOverlaps.length - 1; i2 >= 0 && !ok; i2--) {
                    if (path12.compare(paths2[pathBoundsOverlaps[i2]])) {
        ...
        var Path = PathItem.extend(
          ...
          new function() {
            ...
            return {
              _draw: function(ctx, param, viewMatrix, strokeMatrix) {
                var dontStart = param.dontStart, dontPaint = param.dontFinish || param.clip, style = this.getStyle(), hasFill = style.hasFill(), hasStroke = style.hasStroke(), dashArray = style.getDashArray(), dashLength = !paper3.support.nativeDash && hasStroke && dashArray && dashArray.length;
                if (!dontStart)
                  ctx.beginPath();
              ...
              _drawSelected: function(ctx, matrix) {
                ...
                ctx.stroke();
                drawHandles(ctx, this._segments, matrix, paper3.settings.handleSize);
              }
            };
          }(),
          new function() {
        ...
        PathItem.inject(new function() {
          ...
          function createResult(paths, simplify, path12, path22, options) {
            var result2 = new CompoundPath(Item.NO_INSERT);
            result2.addChildren(paths, true);
            result2 = result2.reduce({ simplify });
            if (!(options && options.insert == false)) {
              result2.insertAbove(path22 && path12.isSibling(path22) && path12.getIndex() < path22.getIndex() ? path22 : path12);
            }
            result2.copyAttributes(path12, true);
            return result2;
          }
          function filterIntersection(inter) {
            return inter.hasOverlap() || inter.isCrossing();
          }
          function traceBoolean(path12, path22, operation, options) {
            if (options && (options.trace == false || options.stroke) && /^(subtract|intersect)$/.test(operation))
              return splitBoolean(path12, path22, operation);
            var _path1 = preparePath(path12, true), _path2 = path22 && path12 !== path22 && preparePath(path22, true), operator = operators[operation];
            ...
            return createResult(paths, true, path12, path22, options);
          }
          function splitBoolean(path12, path22, operation) {
            var _path1 = preparePath(path12), _path2 = preparePath(path22), crossings = _path1.getIntersections(_path2, filterIntersection), subtract = operation === "subtract", divide = operation === "divide", added = {}, paths = [];
            ...
            addPath(_path1);
            return createResult(paths, false, path12, path22);
          }
          ...
          function reorientPaths(paths, isInside, clockwise) {
            ...
            if (length) {
              ...
              for (var i = 0; i < length; i++) {
                var path12 = sorted[i], entry1 = lookup[path12._id], containerWinding = 0, indices = collisions[i];
                if (indices) {
                  var point = null;
                  for (var j = indices.length - 1; j >= 0; j--) {
                    if (indices[j] < i) {
                      point = point || path12.getInteriorPoint();
                      var path22 = sorted[indices[j]];
                      if (path22.contains(point)) {
                        var entry2 = lookup[path22._id];
                        containerWinding = entry2.winding;
                        entry1.winding += containerWinding;
                        entry1.container = entry2.exclude ? entry2.container : path22;
                        break;
                      }
                    }
                  }
          ...
          function propagateWinding(segment, path12, path22, curveCollisionsMap, operator) {
            ...
            for (var i = 0; i < offsets.length && winding.quality < 0.5; i++) {
              ...
              for (var j = 0, l = chain.length; j < l; j++) {
                ...
                if (length <= curveLength) {
                  ...
                  var wind = null;
                  if (operator.subtract && path22) {
                    var otherPath = operand === path12 ? path22 : path12, pathWinding = otherPath._getWinding(pt, dir, true);
                    if (operand === path12 && pathWinding.winding || operand === path22 && !pathWinding.winding) {
          ...
          function tracePaths(segments, operator) {
            ...
            segments.sort(function(seg1, seg2) {
              var inter1 = seg1._intersection, inter2 = seg2._intersection, over1 = !!(inter1 && inter1._overlap), over2 = !!(inter2 && inter2._overlap), path13 = seg1._path, path23 = seg2._path;
              return over1 ^ over2 ? over1 ? 1 : -1 : !inter1 ^ !inter2 ? inter1 ? 1 : -1 : path13 !== path23 ? path13._id - path23._id : seg1._index - seg2._index;
            });
            for (var i = 0, l = segments.length; i < l; i++) {
              var seg = segments[i], valid = isValid(seg), path = null, finished = false, closed = true, branches = [], branch, visited, handleIn;
              if (valid && seg._path._overlapsOnly) {
                var path12 = seg._path, path22 = seg._intersection._segment._path;
                if (path12.compare(path22)) {
                  if (path12.getArea())
                    paths.push(path12.clone(false));
                  visitPath(path12);
                  visitPath(path22);
                  valid = false;
                }
          ...
          return {
            ...
            resolveCrossings: function() {
              ...
              function hasOverlap(seg2, path3) {
                var inter = seg2 && seg2._intersection;
                return inter && inter._overlap && inter._path === path3;
              }
              ...
              if (hasCrossings) {
                ...
                if (clearCurves)
                  clearCurveHandles(clearCurves);
                paths = tracePaths(Base.each(paths, function(path3) {
                  Base.push(this, path3._segments);
                }, []));
              }
        ...
        var Style = Base.extend(new function() {
          var itemDefaults = {
            ...
            initialize: function Style2(style, _owner, _project) {
              this._values = {};
              this._owner = _owner;
              this._project = _owner && _owner._project || _project || paper3.project;
        ...
        var View = Base.extend(
          Emitter,
          {
            ...
            initialize: function View2(project, element) {
              ...
              this._autoUpdate = !paper3.agent.node;
        ...
        var Key = new function() {
          ...
          DomEvent.add(document, {
            keydown: function(event) {
              var key = getKey(event), agent = paper3 && paper3.agent;
        ...
        var ToolEvent = Event.extend({
          ...
          getItem: function() {
            if (!this._item) {
              var result2 = this.tool._scope.project.hitTest(this.getPoint());
              if (result2) {
                var item = result2.item, parent = item._parent;
        ...
        var Tween = Base.extend(Emitter, {
          ...
          _calculate: function(left, operator, right) {
            return paper3.PaperScript.calculateBinary(left, operator, right);
          },
        ...
        new function() {
          ...
          function importNode(node, options, isRoot) {
            ...
            if (isRoot && isElement) {
              rootSize = paper3.getView().getSize();
            ...
            var settings = paper3.settings, applyMatrix = settings.applyMatrix, insertItems = settings.insertItems;
        ...
        Base.exports.PaperScript = function() {
          ...
          function compile(code, options) {
            ...
            var url = options.url || "", sourceMaps = options.sourceMaps, paperFeatures = options.paperFeatures || {}, source = options.source || code, offset = options.offset || 0, agent = paper3.agent, version = agent.versionNumber, offsetCode = false, lineBreaks = /\r\n|\n|\r/mg, map;
        ...
        var paper3 = new (PaperScope.inject(Base.exports, {
          Base,
          Numerical,
          Key,
          DomEvent,
        ...
        if (paper3.agent.node) {
          require_extend()(paper3);
        }
        if (typeof define === "function" && define.amd) {
          define("paper", paper3);
        } else if (typeof module === "object" && module) {
          module.exports = paper3;
        }
        return paper3;
  ...
  import_paper.default.setup(new import_paper.default.Size(100, 300));
  var d1 = "M0,5 L20,30 L40,5 Z";
  var d2 = "M8,7 L60,9 L40,20 Z";
  var path1 = new import_paper.default.Path(d1);
  var path2 = new import_paper.default.Path(d2);
  var result = path2.exclude(path1);
  var svgPathElement = result.exportSVG();
  console.log(svgPathElement.outerHTML);
  var d = svgPathElement.getAttribute("d") || "";
})();
